...
describe('ETH-DAO test', function () {
  ...
  it('token contract is deployed', async function () {
    ...
    // トークンのシンボルがTGTとなっているか確認する
    assert.equal(tokenInfo.symbol, 'TGT');
  });
  ...
  it('token is minted', async function () {
    ...
    // トークンの合計が2e+24となっているか確認する
    assert.equal(Number(tokenInfo.value).toString(), '2e+24');
  });
  ...
  it('token is transfered', async function () {
    ...
    // コントラクトのトークンの合計が2e+23となっているか確認
    assert.equal(fixedBalance, '2e+23');
  });
